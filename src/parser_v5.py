...
# Total samples and positive samples
TESTED_STATISTICS = ['samples', 'positives']
# CSV Headers
      ...
      try:
        ...
        if count_str:
          logging.warning('[L{}] [{}] [Bad {}: {}]'.format(
              j + 2, entry['testedasof'], key, count_str))
        break

      if count:
        data[date]['TT']['total']['tested'][statistic] = count
...
def parse_state_test(raw_data):
  for j, entry in enumerate(raw_data['states_tested_data']):
    ...
    for key, statistic in zip(['totaltested', 'positive'], TESTED_STATISTICS):
      count_str = entry[key].strip()
      try:
        count = int(count_str)
      except ValueError:
        if count_str:
          logging.warning('[L{}] [{}] [Bad {}: {}] {}'.format(
              j + 2, entry['updatedon'], key, count_str, entry['state']))
        break

      if count:
        data[date][state]['total']['tested'][statistic] = count
        ...
        if state in SINGLE_DISTRICT_STATES:
          # District/State name
          district = SINGLE_DISTRICT_STATES[state]
          data[date][state]['districts'][district]['total']['tested'][
              statistic] = count
...
def parse_district_test(reader):
  ...
  for i, row in enumerate(reader):
    ...
    for j in range(6, len(row), 5):
      ...
      for ix, statistic in zip([j, j + 1], TESTED_STATISTICS):
        count_str = row[ix].strip()
        try:
          count = int(count_str)
        except ValueError:
          if count_str:
            logging.warning('[L{} {}] [{}: {}] Bad {}: {}'.format(
                i + 3, column_str(ix), state, district, statistic, count_str))
          break
          ...
          for statistic in TESTED_STATISTICS:
            if statistic in state_data['total']['tested']:
              if 'tested' in curr_data[state][
                ...
                curr_data[state]['total']['tested'][statistic] = state_data[
                    'total']['tested'][statistic]
                if statistic == 'samples':
                  curr_data[state]['meta']['tested']['source'] = state_data[
                      'meta']['tested']['source']
            ...
            for statistic in TESTED_STATISTICS:
              if statistic in district_data['total']['tested']:
                if 'tested' in curr_data[state]['districts'][district][
                  ...
                  # Take today's cumulative to be same as yesterday's
                  # cumulative if today's cumulative is missing
                  curr_data[state]['districts'][district]['total']['tested'][
                      statistic] = district_data['total']['tested'][statistic]
                  if statistic == 'samples':
  ...
  for date in sorted(data):
    ...
    for state in sorted(curr_data):
      ...
      row = [
          ...
          or 0, state_data['total']['tested']['samples'] or ''
      ]
      ...
      for district in sorted(state_data['districts']):
        ...
        row = [
            ...
            district_data['total']['tested']['samples'] or ''
        ]
...
if __name__ == '__main__':
  ...
  logging.info('{:{align}{width}}'.format('PARSER V5 START',
                                          align='^',
                                          width=PRINT_WIDTH))
